...
class delta(FoamFile):
    """
    Representa el archivo 'delta' de OpenFOAM, utilizando el motor
    de plantillas Jinja2 para generar su contenido.
    """
    def __init__(self, second_part=None):
        if second_part != None:
            name_aux = "delta" + "." + second_part
        else:
            name_aux = "delta"
    ...
    def get_editable_parameters(self):
        ...
        return {
            'internalField': {
                'label': 'Campo Interno (delta)',
                'tooltip': 'Define el valor inicial de delta en todo el dominio (0 a 1).',
                'type': 'float',
